...
int main()
{
    ...
    CUDA_CALL(cublasSetMatrix(d,1,sizeof(float),vector,d,vector_gpu,d));
    ...
    for (int i = 1; i < 10; i++)
    {
        ...
        CUDA_CALL(cublasSetMatrix(n,1,sizeof(float),result,n,result_gpu,n));

        timer.start();
        CUDA_CALL(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
            n, 1, d,
            &alpha,
            matrix_gpu, n,
            vector_gpu, d,
