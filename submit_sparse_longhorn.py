...
def submit_weak(ranks, threads, dataset, iter, blocksize, blockleaf, levels, k, leafsize, ltrees, gpu_flag=False, merge=1, overlap=1):
  
  print(ranks)
  OMP="OMP_NUM_THREADS="+str(threads)

  if gpu_flag:
      CMD=f"mpirun -n {ranks} python run_sparse_weak.py -iter {iter} -dataset {dataset} -bs {blocksize} -bl {blockleaf} -levels {levels} -cores {threads} -ltrees {ltrees} -use_gpu {gpu_flag} -leafsize {leafsize} -overlap {overlap} -merge {merge}"
  else:
      CMD=f"mpirun -n {ranks} python run_sparse_weak.py -iter {iter} -dataset {dataset} -bs {blocksize} -bl {blockleaf} -levels {levels} -cores {threads} -ltrees {ltrees} -leafsize {leafsize} -overlap {overlap} -merge {merge}"

  filename = f"_{dataset}_ranks_{ranks}_bs_{blocksize}_bl_{blockleaf}_gpu_{gpu_flag}_threads_{threads}_lt_{ltrees}_levels_{levels}_leaf_{leafsize}_overlap_{overlap}_merge_{merge}_run_{args.run}_weak"
...
if __name__ == '__main__':
  ...
  for ranks in gpu_rank_list:
    for ltrees in local_iterations:
      submit_weak(ranks, 4, "sprand", 5, 256, 512, levels, 32, 1024, ltrees, True, overlap=1, merge=1)
