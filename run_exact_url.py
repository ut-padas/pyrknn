...
for p in size_list:
    ...
    lines.append('#SBATCH -p skx-normal \n')
    lines.append('#SBATCH -N '+str(p)+' \n')
    lines.append("#SBATCH -n "+str(p)+"\n")
    lines.append('#SBATCH --tasks-per-node 1 \n')
    lines.append('#SBATCH -t 00:50:00 \n')
    lines.append('export OMP_NUM_THREADS=48 \n')
    lines.append('source set_env_cpu.sh\n')
    lines.append('ulimit -c unlimited \n')
    lines.append('ibrun -n '+str(p)+' python run_truth_url.py \n')
