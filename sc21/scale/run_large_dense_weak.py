...
def get_gauss_data(rank, N, d, nq):
    ...
    ind = 10
    exd = d
    data = np.random.rand(N, exd)*eps
    spc = 10
    nps = N//spc

    for i in range(spc):
        sphere = np.random.randn(nps, ind)
        idx = np.random.choice(exd, (ind))
        cidx = np.random.choice(ind, (3))
        sphere = normalize(sphere)
        sphere[:, cidx] += np.random.randn()*2
        print(sphere.shape, data.shape, idx.shape)
        data[i*nps:(i+1)*nps, idx] = sphere 
