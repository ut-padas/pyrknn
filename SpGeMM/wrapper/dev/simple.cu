...
__global__ void compute_norm(int* R, int* C, float* V, int* G_Id, float* K, int M_I, int leaf_batch_g, size_t max_ind) {
  ...
  int ind_write = leaf_id_g * gridDim.x * gridDim.x + row * gridDim.x ;
  if (ind_write < 0) printf("leaf = %d , row = %d , ind = %d \n", leaf_id_g, row, ind_write); 
  if (ind_write >= max_ind) printf("leaf = %d , row = %d , ind = %d \n", leaf_id_g, row, ind_write);
  K[ind_write] = norm_i;
...
void gpu_knn(int *R, int *C, float *V, int *G_Id, int M, int leaves, int k, float *knn, int *knn_Id, int max_nnz){
  ...
  //int size_batch_leaves = (pow(2, 32)) / ( 4* pointsperleaf * pointsperleaf) ;
  int arr_size = log2(free / sizeof(float));
  double arr_len = pow(2, arr_size);
 
  printf("log =%d\n", arr_size); 
  array_size = sizeof(float) * arr_len;
  
  
  int size_batch_leaves = arr_len / ( pointsperleaf * pointsperleaf);
  if (size_batch_leaves < 1) size_batch_leaves = 1; 
  ...
  printf("  max_nnz : %d \n", max_nnz);
	printf("blockDim (norms) : (%d, 1)\n", M_I);
  printf("blockGrid (norms) : (%d, %d)\n", M_I, size_batch_leaves);
  ...
  checkCudaErrors(cudaMalloc((void **) &d_K, array_size));
 
  size_t index = sizeof(d_K) / sizeof(float);
  printf("max ind = %lf \n", sizeof(d_K));
 
  ///double size = sizeof(&d_K) / sizeof(float);
  cudaMemGetInfo(&free,&total);
  printf(" %d free from %d \n", free/1024, total/1024); 
  ...
  for (int leaf_id_g = 0; leaf_id_g < num_batch_leaves; leaf_id_g++){
    ...
    compute_norm <<< dimGrid_norm, dimBlock_norm >>>(R, C, V, G_Id, d_K, M_I, leaf_id_g, index);
