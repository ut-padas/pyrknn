...
void f_knnSeq_dense(float* data, int* G_Id, float* K, int* K_Id, int knn, int leaf_id, int pt_id, int leaf_size, int d){
  ...
  //for (int i=0; i< leaf_size; i++){
    ...
    norm = 0.0;
    float norm_i = 0.0;
    for (int s= 0 ; s < d; s++) norm_i += data[perm_i * d + s] * data[perm_i * d + s];
     
    for (int j=0; j < leaf_size; j++){
      int perm_j = G_Id[leaf_size * leaf_id + j];

      
      norm = norm_i;
      for (int s = 0; s < d; s++) norm += data[perm_j* d + s] * data[perm_j * d + s];

      c = 0;
      
      for (int n_i = 0; n_i < d; n_i++) c += data[perm_j * d + n_i] * data[perm_i * d + n_i];
