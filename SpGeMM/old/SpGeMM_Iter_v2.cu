...
__global__ void knn_kernel_sq(int* R, int* C, float* V, int* G_Id, float* Norms , int k_nn, float* KNN_dist, int* KNN_Id, int ppl, int max_nnz, int m, int blockInd, int* block_indices) {
   ...
   */
   ...
   __syncthreads();
    ...
    if (j >= m && i < m) SM_dist[i * 2 * m + j] = (j < k_nn && i < m) ? KNN_dist[leaf_id_g * ppl * k_nn + (b_i * m + i) * k_nn + j + m] : 1e30;
    if (j >= m && i < m) SM[i * 2 * m + j] = (j < k_nn && i < m) ? KNN_Id[leaf_id_g * ppl * k_nn + (b_i * m + i) * k_nn + j + m] : 0;
    if (j >= m && i >= m) SM_dist[i * 2 * m + j] = (j < k_nn) ? KNN_dist[leaf_id_g * ppl * k_nn + (b_j * m + j - m) * k_nn + j + m] : 1e30;
    if (j >= m && i >= m) SM[i * 2 * m + j] = (j < k_nn) ? KNN_Id[leaf_id_g * ppl * k_nn + (b_j * m + j - m) * k_nn + j + m] : 0;
   ...
   //if (col < k_nn){
     ...
     int ind_write = leaf_id_g * ppl * k_nn + (b_i * m + i )* k_nn + j + m;
   ...
   //if (i < k_nn){
   if (j < k_nn -m){
     int ind_write = leaf_id_g * ppl * k_nn + (b_j * m + i - m)* k_nn + j + m;
...
void gpu_knn(int *R, int *C, float *V, int *G_Id, int M, int leaves, int k, float *knn, int *knn_Id, int max_nnz){
  ...
  knn_kernel_tri <<< dimGrid_tri, dimBlock_tri >>>(R, C, V, G_Id, d_Norms, k, knn, knn_Id, ppl, max_nnz, m);
  for (int blockInd = 0; blockInd < num_blocks_tri - 1; blockInd++){  
    //checkCudaErrors(cudaDeviceSynchronize());
    
    //checkCudaErrors(cudaDeviceSynchronize());
    knn_kernel_sq <<< dimGrid_sq, dimBlock_sq >>>(R, C, V, G_Id, d_Norms, k, knn, knn_Id, ppl, max_nnz, m ,blockInd, d_block_indices);
    checkCudaErrors(cudaDeviceSynchronize());
  } 
  //size_t free, total;
  cudaMemGetInfo(&free, &total);
