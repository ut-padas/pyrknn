...
__global__ void MergeVer_v2(float* KNN, int* KNN_Id, int k_nn, int ppl, int blockInd, float* d_temp_knn, int* sort_arr, int* G_Id, bool init, int M, int bl, int sizebleaves, int partsize){
  ...
  int col = blockIdx.x;
  int leafId_local = blockIsx.z * blockDim.y + blockIdx.y;
  ...
  int ind_tmp = (init) ? leafId_local * ppl * partsize + col * partsize + j : leaf_id_g * k_nn * size_part + j * size_part + col;
