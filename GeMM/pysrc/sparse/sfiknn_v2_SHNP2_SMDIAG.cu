...
__global__ void ComputeTriDists(int* R, int* C, float* V, int* G_Id, float* Norms , int k_nn, float* KNN_dist_tmp, int ppl, int bl, int sizebleaves, int partsize) {
  ...
  int block = blockIdx.x / (partsize/2);
  int rowId = blockIdx.x - block * (partsize/2);
  int colId = threadIdx.x;

  int last_block = ppl / partsize;

  int rowId2 = partsize - rowId - 1;

  bool accpair = true;
  //if (last_block == block && rowId + last_block * partsize >= ppl) accpair = false;
  //if (! accpair) return;
  //if (last_block == block && rowId2 + last_block * partsize >= ppl) accpair = false;

  

  if (rowId >= colId){
    colId = partsize - colId - 1;
    rowId = partsize - rowId - 1;
    //int rowId2 = rowId;
  } else {
    colId -= 1;
    //int rowId2 = partsize - rowId - 1;
  ...
  int rem_len = ppl - last_block * partsize;

  
  int g_rowId2 = leafId_g * ppl + block * partsize + rowId2;
  int perm_i2 = g_rowId2;
  int ind0_i2 = R[perm_i2];
  int ind1_i2 = R[perm_i2 + 1];
  int nnz_i2 = ind1_i2 - ind0_i2;

  int shift = (rowId >= partsize /2) ? SM_SIZE_2 : 0;
  
  
  for (int n_i = ind; n_i < nnz_i; n_i+= blockDim.x) SM[n_i] = C[ind0_i + n_i];
  
  
  for (int n_i = ind; n_i < nnz_i2; n_i+= blockDim.x) SM[SM_SIZE_2 + n_i] = C[ind0_i2 + n_i];
  ...
  if (colId >= rowId){
    if (block != last_block || (rowId < rem_len && colId < rem_len)){ 
			float c_tmp = 0.0;
			//changed 
			...
			if (nnz_i > 0 && nnz_j >0){
				for (int pos_k = 0; pos_k < nnz_j; pos_k++){
					...
					for (int l = nnz_i - ret; l > 1; l -= floorf(l/2.0)){
						...
						ret = (SM[shift + testInd] <= k) ? testInd : ret;
					}
...
__global__ void S_ComputeTriDists_last(int* R, int* C, float* V, int* G_Id, float* Norms , int k_nn, float* KNN_dist_tmp, int ppl, int bl, int sizebleaves, int partsize, int block, int rem_len) {
  ...
  if (rowId >= colId){
    colId = rem_len - colId - 1;
    rowId = rem_len - rowId - 1;
    //int rowId2 = rowId;
  } else {
    colId -= 1;
    //int rowId2 = partsize - rowId - 1;
  ...
  int tmp_0, tmp_1, ind_jk, k, ret, testInd;
  if (colId >= rowId){
    if (rowId < rem_len && colId < rem_len){ 
			float c_tmp = 0.0;
			//changed 
...
void sfi_leafknn(int *d_R, int *d_C, float *d_V, int *d_GId, int M, int leaves, int k, float *d_knn, int *d_knn_Id, int deviceId){
  ...
  float dt1, dt2, dt3, dt4, dt5, dt6, dt7, dt8, dt9, dt_tmp, dt10;
  ...
  int blockDim_tri = partsize + 1;
  int blockDim_tri_last = rem_len + 1;
  //if (blockDim_tri > SM_SIZE_1) blockDim_tri = SM_SIZE_1;

  //int size_tri_last = (rem_len > 32) ? 32 : rem_len;
  int size_tri_last = rem_len;

  int numtri = ppl / partsize;
  int gridDimTri = (rem_len == 0) ? ppl/2 : (ppl-rem_len)/2;
  int gridDimTri_last = rem_len/2;

  dim3 BlockDistTri(blockDim_tri, 1, 1);
  dim3 GridDistTri(gridDimTri, batch_leaves_1, batch_leaves_2);
  
  dim3 BlockDistTri_last(blockDim_tri_last, 1, 1);
  dim3 GridDistTri_last(gridDimTri_last, batch_leaves_1, batch_leaves_2);
  ...
  */
  ...
  float tmp_size = size_tmp/1e9;
  float tmp_m2 = m2/1e9;
  //float tmp2 = ceil(tmp_size/tmp_m2);
  ...
  for (int bl = 0; bl < numbleaves; bl++){
    ...
    if (rem_len > 0) {
      S_ComputeTriDists_last <<< GridDistTri_last, BlockDistTri_last >>>(d_R, d_C, d_V, d_GId, d_Norms, k, d_temp_knn, ppl, bl, sizebleaves, partsize, num_blocks_tri, rem_len);
      checkCudaErrors(cudaDeviceSynchronize());
    }
    int size_v = ppl;
  ...
  checkCudaErrors(cudaEventElapsedTime(&dt4, t3, t_diagdist));
  checkCudaErrors(cudaEventElapsedTime(&dt8, t4, t9));
  checkCudaErrors(cudaEventElapsedTime(&dt9, t0, t9));
  checkCudaErrors(cudaEventElapsedTime(&dt10, t_diagdist, t4));
