...
__global__ void MergeHoriz(float* KNN, int* KNN_Id, float* Norms, int k_nn, int ppl, int blockInd, float* d_temp_knn, int* G_Id, bool init, int bl, int sizebleaves, int partsize){
  ...
  int size_last = size_part - num_batches * (size_sort - k_nn) + k_nn ;
  
  int log2_size_last = 1;
  int tmp_log = size_last;
  while (tmp_log > 1) {
    tmp_log /= 2; 
    log2_size_last++;
  }
  int act_threads = 1 << log2_size_last;
  act_threads /= 2;
  ...
  for (int col_batch = 0; col_batch < num_batches; col_batch++){
    for (int j_tmp = j; j_tmp < size_sort; j_tmp += blockDim.x){
      ...
      if (col_batch == 0 && j_tmp < k_nn){
        ...
        int ind_tmp = leafId_local * partsize * size_part + row_l + (colId_leaf - (partsize) * (blockInd+1)) * partsize;
  ...
  if (j >= act_threads) return;

  if (size_last > k_nn){ 
    if (j >= size_last - k_nn){
      int colId_leaf = partsize * blockInd + num_batches * (size_sort - k_nn) + j;
      //int size_tmp = size_part - partsize;
      int ind_tmp = leafId_local * partsize * size_part + row_l + (colId_leaf - (partsize) * (blockInd+1)) * partsize;
      int g_colId_J = leafId_g * ppl + colId_leaf;
        
      tmp_f = (j < size_last - k_nn) ? -2 * d_temp_knn[ind_tmp] + norm_i + Norms[leafId_g * ppl + colId_leaf] : 1e30;
      SM_dist[j + k_nn] = (tmp_f > 0) ? sqrt(tmp_f) : 0.0;
      SM_Id[j + k_nn] = G_Id[g_colId_J];
    }


    __syncthreads();
  
		for (int g = 2; g <= 2 * act_threads; g *=2){
			for (int l = g/2; l > 0; l /= 2){
  ...
  */
  ...
  for (int bl = 0; bl < numbleaves; bl++){
    ...
    if (rem_len > 0){
      ...
      float tmp_f = size_sort;
      int log2_size_sort = log2(tmp_f);
      int size_sort_pow2 = 1 << log2_size_sort;
      if (size_sort_pow2 < size_sort) size_sort_pow2 *= 2;

			while (size_sort_pow2 > SM_SIZE_2) size_sort_pow2 /= 2;
			
			int blocksize = size_sort_pow2/2;
      ...
      //printf("BlockInd = %d , size_sort = %d \n", blockInd, 2 * blocksize);    
    ...
    */
      ...
      checkCudaErrors(cudaEventSynchronize(t6));
			...
			MergeHoriz <<< GridMergeHoriz, BlockMergeHoriz, 0, streams[0]>>> (d_knn, d_knn_Id, d_Norms, k, ppl, blockInd, d_temp_knn, d_GId, false, bl, sizebleaves, partsize); 
			//MergeHoriz <<< GridMergeHoriz, BlockMergeHoriz>>> (d_knn, d_knn_Id, d_Norms, k, ppl, blockInd, d_temp_knn, d_GId, false, bl, sizebleaves, partsize); 
