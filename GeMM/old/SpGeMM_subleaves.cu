...
__global__ void find_neighbor(float* knn, int* knn_Id, float* K, int* G_Id, int k, int M_I, int leaf_batch_g, int M){
...
void gpu_knn(int *R, int *C, float *V, int *G_Id, int M, int leaves, int num_subleaves, int k, float *knn, int *knn_Id, int max_nnz){
  ...
  //int size_batch_I = (pointsperleaf + m_i - 1)/m_i;
	int size_batch_I = (pointspersubleaf + m_i - 1)/m_i;

	//int size_batch_J = (pointsperleaf + m_j - 1) / m_j;
	int size_batch_J = (pointspersubleaf + m_j - 1) / m_j;
  ...
  dim3 dimGrid(size_batch_J, size_batch_I, size_batch_leaves * num_subleaves * num_subleaves); 
  ...
  for (int leaf_id_g = 0; leaf_id_g < num_batch_leaves; leaf_id_g++){
    ...
    find_neighbor <<< dimGrid_n, dimBlock_n >>>(knn, knn_Id, d_K, G_Id, k, M_I, leaf_id_g, M);
    //checkCudaErrors(cudaDeviceSynchronize());
  ...
  printf(" blockGrid (distance) : (%d,%d,%d) \n", size_batch_J, size_batch_I, size_batch_leaves * num_subleaves * num_subleaves);
  printf(" blockDim (find knn) : (%d,%d) \n", M_I, 1);
...
int main(int argc, char **argv)
{
  ...
  //, del_t2, del_t3;
    ...
    int num_subleaves = 2;       // number of subleaves, 
    d = 10000;
    int k = 32;
    nnzperrow = 256;
    ...
    */
    ...
    gpu_knn(d_R, d_C, d_V, d_G_Id, M, leaves, num_subleaves, k, d_knn, d_knn_Id, max_nnz);
