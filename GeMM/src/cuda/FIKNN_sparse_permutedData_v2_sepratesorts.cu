...
__global__ void knn_kernel_sortHoriz(float* KNN, int* KNN_Id, int k_nn, int ppl, int blockInd, float* d_temp_knn, int* sort_arr, int* sort_arr_part, int steps, int* G_Id, bool init){
...
void FIKNN_sparse_gpu(int *R, int *C, float *V, int *G_Id, int M, int leaves, int k, float *knn, int *knn_Id, int max_nnz, int iternum){
  ...
  */
  ...
  for (int blockInd = 0; blockInd < num_iters; blockInd++){
    ...
    knn_kernel_Dist <<< dimGrid_sq, dimBlock_dist >>> (d_R, d_C, d_V, d_GId, d_Norms, k, ppl, max_nnz, blockInd, d_temp_knn);
    checkCudaErrors(cudaDeviceSynchronize());
    
    knn_kernel_sortHoriz <<< dimGrid_sq, dimBlock_sortHoriz >>> (d_knn, d_knn_Id, k, ppl, blockInd, d_temp_knn, d_arr, d_arr_part, steps, d_GId, false); 
    checkCudaErrors(cudaDeviceSynchronize());
