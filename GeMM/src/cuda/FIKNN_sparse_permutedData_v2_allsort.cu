...
#define SM_SIZE_MAX_1 4096
#define SM_SIZE_MAX_2 2048
...
__global__ void knn_kernel_A(int* R, int* C, float* V, int* G_Id, float* Norms, int k_nn, float* KNN_dist, int* KNN_Id, int ppl, int max_nnz, int blockInd, int* sort_arr, int* sort_arr_part, int steps, float* d_knn_temp) {
  ...
  __shared__ float SM_dist[SM_SIZE_MAX_2];
  __shared__ int SM_Id[SM_SIZE_MAX_2];
  ...
  int num_batches = 1;
  while (size_sort_f > SM_SIZE_MAX_2) {
  ...
  for (int init_write = j; init_write < SM_SIZE_MAX_2; init_write += blockDim.x) SM_dist[init_write] = 1e30;
  ...
  for (int col_batch = 0; col_batch < num_batches; col_batch++){
    ...
    for (int step = 0 ; step < steps; step++){
      for (int j_tmp =j; j_tmp < size_sort/2.0; j_tmp += blockDim.x){
...
void FIKNN_sparse_gpu(int *R, int *C, float *V, int *G_Id, int M, int leaves, int k, float *knn, int *knn_Id, int max_nnz, int iternum){
  ...
  */
  ...
  float SM_SIZE_MAX_2_f = SM_SIZE_MAX_2;
  int n_s = log2(SM_SIZE_MAX_2_f) *(log2(SM_SIZE_MAX_2_f)+1) /2;
  ...
  for (int blockInd = 0; blockInd < num_iters; blockInd++){
    ...
    while (size_sort > SM_SIZE_MAX_2) size_sort = ceil((size_sort+k)/2);
    ...
    int blocksize_thread = blocksize;
    while (blocksize_thread > SM_SIZE_1) blocksize_thread /= 2;
 
    dim3 dimBlock_sq(blocksize_thread, 1, 1);
