...
def run():
    ...
    #Build Forest
    forest = RKDForest(pointset=X, levels=4, leafsize=256, location="CPU", ntrees=1, sparse=sparse, N=n_local, d=d)
    forest.build()
    ...
    print("Loading Exact Search")
    #truth_ind = np.load('url_nborID_1000.bin.npy')
    #truth_dist = np.load('url_nborID_1000.bin.npy')
    #truth = (truth_ind, truth_dist)
    #truth = None
    ...
    trees_per_batch = 1
    approx = forest.aknn_all_build(k, ntrees=trees_per_batch, cores=48, truth=truth, until=True)
    print("Finished Approximate Search")

    if rank == 0:
        #Show results
        print("Approximation Solution", approx, flush=True)
