...
__global__ void FusedLeafKNN_seq(int* R_ref, int* C_ref, float* V_ref, int* R_q, int* C_q, float* V_q, int* local_leafIds, float* Norms_q, float* Norms_ref, int const k_nn, int const ppl, int const d, int* QId, float* NDist, int* NId, int* glob_leafIds, int steps, int* SortInd, int* StepLen, int* StepStart, int* tidIdMap, int* tidSortDir){
  ...
  for (int n = tid; n < k_nn; n += blockDim.x) {
    SM_Dist[n] = NDist[q * k_nn + n];
    SM_Id[n] = NId[q * k_nn + n];
  }
  ...
  for (int pt = tid; pt < ppl; pt += blockDim.x){
    ...
    int leafId_g = glob_leafIds[leafId];
    SM_Id[tid + k_nn] = leafId_g * ppl + pt;
    //if (q == 0) printf("D[%d] = %.4f at %d \n", tid+k_nn, SM_Dist[tid+k_nn], SM_Id[tid+k_nn]);
     
    // check duplicates
    int index = leafId_g * ppl + pt;
    ...
    int check = pt / blockDim.x;
		//if (tid < blocksize && q == 0 && check == 0) printf("b = %d, processing D[%d] = %.4f at %d \n",pt/blockDim.x, tid, SM_Dist[tid], SM_Id[tid]);
    //if (tid < blocksize && q == 0 && check == 0) printf("b = %d, processing D[%d] = %.4f at %d \n",pt/blockDim.x, tid+blocksize, SM_Dist[tid+blocksize], SM_Id[tid+blocksize]);
    ...
    //if (q == 0) printf("b = %d, Sorted D[%d] = %.4f at %d \n",check, tid, SM_Dist[tid], SM_Id[tid]);
