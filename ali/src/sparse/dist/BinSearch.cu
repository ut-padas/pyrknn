...
__global__ ComputeDists(int* R_ref, int* C_ref, float* V_ref, int* R_q, int* C_q, float* V_q, int* leafIds, float* Norms_q, float* Norms_ref, , int const k_nn, float* KNN_tmp, int ppl, int const p){
  ...
  */
  ...
  __shared__ int SM_search_start[SM_SIZE_12];
  __shared__ int SM_search_stop[SM_SIZE_12];
