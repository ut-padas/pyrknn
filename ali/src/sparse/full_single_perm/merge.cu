...
__global__ void S_MergeHorizNP2(float* KNN, int* KNN_Id, int const k_nn, int const ppl, float* tmp_knnDist, int* glob_pointIds, int  const steps, int* QId, int* SortInd, int* step_len, int* step_start, int* tid_idmap, int* tid_sortdir, int* local_leafIds){
  ...
  int const leafId_local = local_leafIds[q];
  ...
  for (int col_batch = 0; col_batch < num_batches; col_batch++){
    for (int j_tmp = tid; j_tmp < size_sort; j_tmp += blockDim.x){
      ...
      int colId_leaf = (col_batch == 0) ? j_tmp - k_nn : col_batch * (size_sort - k_nn) + j_tmp;
      ...
      if (col_batch == 0 && j_tmp < k_nn){
        ...
        //if (QId[q] == 5) printf("SM_dist[%d] = %.4f at %d , readfrom %d \n", j_tmp, SM_dist[j_tmp], SM_Id[j_tmp], ind_read);
      
      } else if (colId_leaf < ppl && j_tmp >= k_nn){

        int ind_tmp = q * ppl + colId_leaf;
        int g_colId_J = glob_pointIds[leafId_local * ppl + colId_leaf];

        SM_dist[j_tmp] = tmp_knnDist[ind_tmp];
        SM_Id[j_tmp] = g_colId_J;
        //if (QId[q] == 5 && g_colId_J == 2145034) printf("SM_dist[%d] = %.4f at %d , colId = %d , readfrom %d \n", j_tmp, SM_dist[j_tmp], SM_Id[j_tmp], colId_leaf, ind_tmp);
      }
      //if (QId[q] == 5 && SM_Id[j_tmp] == 2145034) printf("D[%d] = %.4f at %d , q = %d , QId = %d, local_leaf[QId] = %d, glob_leafId = %d \n", j_tmp, SM_dist[j_tmp], SM_Id[j_tmp], q, QId[q], local_leafIds[q], glob_leafIds[local_leafIds[q]]);
      //if (QId[q] == 986 && SM_Id[j_tmp] == 2145034) printf("D[%d] = %.4f at %d , q = %d , QId = %d, local_leaf[QId] = %d,\n", j_tmp, SM_dist[j_tmp], SM_Id[j_tmp], q, QId[q], local_leafIds[q]);
    }
    __syncthreads();
  ...
  __syncthreads();
  //if (QId[q] == 5 && tid < 3 * k_nn) printf("sorted SM_dist[%d] = %.4f at %d \n", tid, SM_dist[tid], SM_Id[tid]);
