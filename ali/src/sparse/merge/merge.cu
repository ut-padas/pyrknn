...
__global__ void S_MergeHorizNP2(float* KNN, int* KNN_Id, int const k_nn, int const ppl, float* tmp_knnDist, int* glob_leafIds, int  const steps, int* QId, int* SortInd, int* step_len, int* step_start, int* tid_idmap, int* tid_sortdir, int* local_leafIds){
  ...
  int const leafId_g = glob_leafIds[local_leafIds[q]];
  ...
  for (int col_batch = 0; col_batch < num_batches; col_batch++){
    for (int j_tmp = tid; j_tmp < size_sort; j_tmp += blockDim.x){
      ...
      if (col_batch == 0 && j_tmp < k_nn){
        ...
        //if (QId[q] == 0) printf("SM_dist[%d] = %.4f at %d , readfrom %d \n", j_tmp, SM_dist[j_tmp], SM_Id[j_tmp], ind_read);
  ...
  //if (QId[q] == 0) printf("SM_dist[%d] = %.4f at %d \n", tid, SM_dist[tid], SM_Id[tid]);
  //if (QId[q] == 0) printf("SM_dist[%d] = %.4f at %d \n", tid+blockDim.x, SM_dist[tid+blockDim.x], SM_Id[tid+blockDim.x]);
