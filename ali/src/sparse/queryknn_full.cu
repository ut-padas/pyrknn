...
void query_leafknn(int *R_ref, int *C_ref, float *V_ref, int *R_q,  int * C_q, float *V_q, int *GId, int N, int const leaves, int const k, float *d_knn, int *d_knn_Id, int const deviceId, int const verbose, int const nq, int leafIds, int const dim, int const avgnnz, int const num_search_leaves){
  ...
  checkCudaErrors(cudaEventCreate(&t__memalloc));
  ...
  size_t tmp_NDist_size = sizeof(float) * k_nn * nq;
  ...
  ComputeDists <<< GridDist, BlockDist >>> (R_ref, C_ref, V_ref, R_q, C_q, V_q, leafIds, Norms_q, Norms_ref, tmp_Ndist, ppl, qsearch_ind, size_search, dim);
