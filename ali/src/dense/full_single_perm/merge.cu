...
__global__ void S_MergeHorizNP2(float* KNN, int* KNN_Id, int const k_nn, int const ppl, float* tmp_knnDist, int* glob_pointIds, int  const steps, int* QId, int* SortInd, int* step_len, int* step_start, int* tid_idmap, int* tid_sortdir, int* local_leafIds, float* Norm_q, float* Norm_ref){
  ...
  for (int col_batch = 0; col_batch < num_batches; col_batch++){
    for (int j_tmp = tid; j_tmp < size_sort; j_tmp += blockDim.x){
      ...
      if (col_batch == 0 && j_tmp < k_nn){
        ...
        float norm_ref = Norm_ref[q * ppl + colId_leaf];
        float tmp = -2 * tmp_knnDist[ind_tmp] + norm_q + norm_ref;
        if (tmp < 1e-8) tmp = 0.0; 
        SM_dist[j_tmp] = tmp;
        SM_Id[j_tmp] = g_colId_J;
        //if (QId[q] == 0 && j_tmp < 100) printf("D[%d] = %.4f at %d , norm_q = %.4f, norm_ref = %.4f , inner = %.4f \n", j_tmp, SM_dist[j_tmp], SM_Id[j_tmp], norm_q, norm_ref, tmp_knnDist[ind_tmp]);
      }
    }
    __syncthreads();
