...
void query_leafknn_seqsearch(float *X_ref, float *X_q, int *QId, int const ppl, int const leaves, int const k, float *NDist, int *NId, int const deviceId, int const verbose, int const nq, int const dim, int *glob_leafIds, int num_search_leaves, int *local_leafIds){
  ...
  */
  ...
  CHECK_CUBLAS(cublasSgemmStridedBatched( handle, CUBLAS_OP_T, CUBLAS_OP_N, 
                                          1, 1, dim,
                                          &oneFloat, X_ref, dim, dim,
                                          X_ref, dim, dim,
                                          &zeroFloat, Norms_ref, oneInt, oneInt, num_search_leaves*ppl));
  ...
  CHECK_CUBLAS(cublasSgemmStridedBatched( handle, CUBLAS_OP_T, CUBLAS_OP_N, 
                                          ...
                                          X_ref, dim, ppl * dim,
                                          &zeroFloat, tmp_NDist, nq, ppl, num_search_leaves));
  ...
  S_MergeHorizNP2<<< GridMerge, BlockMerge >>> (NDist, NId, k, ppl, tmp_NDist, glob_leafIds, steps, QId, SortInd, StepLen, StepStart, tidIdMap, tidSortDir, local_leafIds, Norms_q, Norms_ref);
