...
void merge_neighbors_gpu(float*, const float*, int*, const int*, int, int,
    float*, int*, int, float&, bool debug=false);

void merge_neighbors_gpu(float*, int*, int, int, int);
...
int main(int argc, char *argv[]) {
  ...
  float t_kernel = 0.;
  merge_neighbors_gpu(D1.data(), D2.data(), I1.data(), I2.data(), m, k, 
  ...
  merge_neighbors_gpu(D_test.data(), I_test.data(), m, 2*k, k);
  
  std::cout<<"Dist error: "<<(D_test.leftCols(k)-nborDistCPU).norm()<<"\n"
    <<"ID error: "<<(I_test.leftCols(k)-nborIDCPU).norm()<<"\n"
    <<std::endl;
