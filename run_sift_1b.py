...
def get_gauss_data(rank, N, d, nq):
    ...
    filename = "/scratch/06081/wlruys/bigann_base.bvecs"
    t = time.time()
    d    =128
    vsz = 4+d
    start = rank*N
...
def run():
    ...
    if args.overlap:
        approx = forest.overlap_search(k, ntrees=args.iter, ltrees = args.ltrees, truth=truth, cores=args.cores, blocksize=args.bs, blockleaf=args.bl, merge_flag=args.merge, threshold=0.95)
    else:
